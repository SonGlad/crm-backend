...
const updateLeadTimeZone = async (req, res) => {
  const { leadId } = req.params;
  const { timeZone: bodyTimeZone } = req.body;
  ...
  switch (authBranch) {
    case "Main":
      ...
      if (!lead || !lead.branch) {
        return res
          .status(400)
          .send({ message: "Lead not found or branch not specified in lead" });
      }
      ...
      switch (lead.branch) {
        case "Office1":
          ...
          updatedLead = await Office1Leads.findOneAndUpdate(
            { _id: leadId },
            {
              ...
              latestComment: {
                ...
                createdAt: Date.now(),
                comment: `The lead time zone was changed from ${prevLead.timeZone} to ${bodyTimeZone} by the user ${user.username}`,
              },
            },
            { new: true }
          );
          await AllCommentsSchema.create({
            ...
            createdAt: Date.now(),
            comment: `The lead time zone was changed from ${prevLead.timeZone} to ${bodyTimeZone} by user ${user.username}`,
          });
          break;
        ...
        case "Office2":
          ...
          updatedLead = await Office2Leads.findOneAndUpdate(
            { _id: leadId },
            {
              timeZone: bodyTimeZone,
              latestComment: {
        ...
        default:
          return res
            .status(400)
            .send({ message: "Lead branch not specified in lead" });
      }
      break;
    ...
    case "Office1":
      ...
      await AllCommentsSchema.create({
        ...
        createdAt: Date.now(),
        comment: `The lead status was changed from ${prevLead.timeZone} to ${bodyTimeZone} by user ${user.username}`,
      });
      break;
...
module.exports = {
  updateLeadTimeZone: ctrlWrapper(updateLeadTimeZone),
};
