...
const updateLeadLastCall = async (req, res) => {
  const { leadId } = req.params;
  const { lastCall: bodylastCall } = req.body;
      ...
      switch (lead.branch) {
        case "Office1":
          ...
          updatedLead = await Office1Leads.findOneAndUpdate(
            { _id: leadId },
            {
              ...
              latestComment: {
                ...
                createdAt: Date.now(),
                comment: `The lead last call was changed to ${bodylastCall} by the user ${user.username}`,
              },
            },
            { new: true }
          );
          await AllCommentsSchema.create({
            ...
            createdAt: Date.now(),
            comment: `The lead last call was changed to ${bodylastCall} by user ${user.username}`,
          });
          break;
                  ...
                  updatedLead = await Office2Leads.findOneAndUpdate(
                      { _id: leadId },
                      {
                          ...
                          latestComment: {
                              ...
                              createdAt: Date.now(),
                              comment: `The last call zone was changed to ${bodylastCall} by user ${user.username}`,
                          },
                      },
                      { new: true }
                  );
...
module.exports = {
  updateLeadLastCall: ctrlWrapper(updateLeadLastCall),
};
