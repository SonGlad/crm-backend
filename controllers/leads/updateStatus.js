      ...
      switch (lead.branch) {
        case "Office1":
          ...
          updatedLead = await Office1Leads.findOneAndUpdate(
            { _id: leadId },
            {
              ...
              latestComment: {
                ...
                createdAt: Date.now(),
                comment: `The lead status was changed from ${prevLead.status} to ${bodyStatus} by the user ${user.username}`,
              },
            },
            { new: true }
          );
          await AllCommentsSchema.create({
            ...
            createdAt: Date.now(),
            comment: `The lead status was changed from ${prevLead.status} to ${bodyStatus} by user ${user.username}`,
          });
          break;
