...
const updateLeadNextCall = async (req, res) => {
  const { leadId } = req.params;
  const { nextCall: bodyNextCall } = req.body;
          ...
          updatedLead = await Office1Leads.findOneAndUpdate(
            { _id: leadId },
            {
              ...
              latestComment: {
                ...
                createdAt: Date.now(),
                comment: `The lead next call was changed to ${bodyNextCall} by the user ${user.username}`,
              },
            },
            { new: true }
          );
                  ...
                  updatedLead = await Office2Leads.findOneAndUpdate(
                      { _id: leadId },
                      {
                          ...
                          latestComment: {
                              ...
                              createdAt: Date.now(),
                              comment: `The next call zone was changed to ${bodyNextCall} by user ${user.username}`,
                          },
                      },
                      { new: true }
                  );
...
module.exports = {
  updateLeadNextCall: ctrlWrapper(updateLeadNextCall),
};
